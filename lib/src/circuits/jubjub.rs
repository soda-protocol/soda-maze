...
use ark_ec::models::{TEModelParameters, twisted_edwards_extended::GroupProjective};
...
pub struct JubjubEncrypt<P, FH, FHG>
where
    P: TEModelParameters,
    ...
    P::BaseField: PrimeField,
{
    ...
    pubkey: GroupProjective<P>,
    nonce_bits: Vec<bool>,
    commitment: (GroupProjective<P>, GroupProjective<P>),
    _p: PhantomData<FHG>,
}

impl<P, FH, FHG> JubjubEncrypt<P, FH, FHG>
where
    P: TEModelParameters,
    FH: FieldHasher<P::BaseField>,
    FHG: FieldHasherGadget<P::BaseField, FH>,
...
mod tests {
    ...
    fn test_jubjub_encryption() {
        ...
        // gen params
        let params = JubjubConstParams::<_, PoseidonHasher<_>> {
            nullifier_params: Rc::new(nullifier_params),
            pubkey,
        };

        // gen origin inputs
        let orig_in = JubjubOriginInputs {
            nonce: Fr::rand(rng),
        };
        ...
        let jubjub = JubjubEncrypt::<_, _, PoseidonHasherGadget<_>>::new(
            params.nullifier_params,
            params.pubkey,
            priv_in.nonce_bits,
            pub_in.commitment,
        );
        jubjub.synthesize(cs.clone(), leaf_index, secret).unwrap();
